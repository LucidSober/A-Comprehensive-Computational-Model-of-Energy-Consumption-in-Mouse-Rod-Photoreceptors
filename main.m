...
Y0 = pack_states_to_vector(states); % Pack the initial states into a vector for the ODE solver.

%% 2. Simulation Setup
t_total = 5.0; % Total simulation time in seconds
flash_start = 0.5; % Time of flash onset in seconds
flash_duration = 0.02; % Duration of the light flash in seconds
light_intensities = [1.7, 4.8, 15.2, 39.4, 125, 444, 1406, 4630]; % Different light intensities to simulate (photons/um^2/s)

% Add light stimulus parameters to the params struct
...
options = odeset('RelTol', 1e-6, 'AbsTol', 1e-9, ...
                 'MaxStep', 0.001, ...  % Limit the maximum step size
                 'InitialStep', 1e-6); % Use a small initial step

%% 3. Find the Steady State in Darkness
fprintf('2. Finding the steady state in dark conditions...\n');
params.Jhv = 0; % Set light intensity to zero for dark adaptation

% Solve in steps to reach the steady state, preventing numerical instability
tspan_ss1 = [0 0.1];  % First, run for 0.1 seconds
[~, Y_ss1] = ode15s(@(t,Y) derivatives(t, Y, params), tspan_ss1, Y0, options);

tspan_ss2 = [0.1 1];  % Then, run until 1 second
[~, Y_ss2] = ode15s(@(t,Y) derivatives(t, Y, params), tspan_ss2, Y_ss1(end,:)', options);

tspan_ss3 = [1 5];  % Finally, run until 5 seconds
[~, Y_ss3] = ode15s(@(t,Y) derivatives(t, Y, params), tspan_ss3, Y_ss2(end,:)', options);

Y_steady_state = Y_ss3(end, :)'; % This is the final steady-state vector
...
fprintf('\nSteady-State Check:\n');
fprintf('  Vm = %.2f mV (Expected: ~-36.2 mV)\n', steady_state_check.Vm);
fprintf('  cGMP = %.2f uM (Expected: ~6.5 uM)\n', steady_state_check.cGMP);
fprintf('  Caos = %.3f uM (Expected: ~0.25 uM)\n', steady_state_check.Caos);
fprintf('  Cais = %.3f uM (Expected: ~0.1 uM)\n', steady_state_check.Cais);
fprintf('  Ki = %.1f mM (Expected: ~140 mM)\n', steady_state_check.Ki);
fprintf('  Nai = %.1f mM (Expected: ~5 mM)\n', steady_state_check.Nai);

% Calculate the dark current
states_ss = unpack_vector_to_states(Y_steady_state);
currents_ss = calculate_currents(states_ss, params);
fprintf('  Dark Current = %.2f pA (Expected: ~-15 pA)\n', currents_ss.I_CNG + currents_ss.I_NCKX);

fprintf('   Steady state achieved!\n\n');

%% 4. Run Simulations for Different Light Intensities
fprintf('3. Starting light response simulations...\n');
...
for i = 1:length(light_intensities)
    ...
    fprintf('   Simulating %d/%d: Light Intensity = %.1f photons/um^2/s\n', i, length(light_intensities), J_hv);
    
    % Set the current light intensity
    params.Jhv = J_hv;
    
    % Run the full simulation (the derivatives function will handle the stimulus timing)
    [t_full, Y_full] = ode15s(@(t,Y) derivatives(t, Y, params), [0, t_total], Y_steady_state, options);
    
    % Unpack and store results for this simulation
    t_results{i} = t_full;
    results{i} = unpack_solution_to_results(t_full, Y_full, params, J_hv);
end

fprintf('\n4. Printing data at key time points...\n');
analyze_and_print_results(results, t_results, light_intensities, params);

%% 5. Plotting
fprintf('5. Plotting results...\n');
plot_final_results_figure2(results, t_results, params);
